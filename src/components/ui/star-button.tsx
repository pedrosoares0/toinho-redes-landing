 ...
 export function StarButton({ 
   ...
   return ( 
     <button 
       ...
       ref={pathRef} 
       className={cn( 
         "relative z-[3] overflow-hidden h-14 px-8 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-all duration-300 disabled:pointer-events-none disabled:opacity-50 group/star-button hover:bg-brand-blue/90", 
         className, 
       )} 
       {...props} 
     > 
       <div 
         className="absolute aspect-square inset-0 animate-star-btn bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)] opacity-0 group-hover/star-button:opacity-100 transition-opacity" 
         style={ 
           { 
             offsetPath: "var(--path)", 
             offsetDistance: "0%", 
       ...
       /> 
       <div 
         className="absolute inset-0 border-white/30 z-[4] overflow-hidden rounded-[inherit] transition-colors group-hover/star-button:border-transparent" 
         style={{ borderWidth: "var(--border-width)" }} 
         aria-hidden="true" 
       > 
         <StarBackground color={backgroundColor} /> 
       </div> 
       <span className="z-10 relative text-white inline-block font-black uppercase tracking-[0.2em] group-hover/star-button:scale-105 transition-transform"> 
         {children} 
       </span> 
     </button> 
   ); 
